import { Heart, Leaf, MessageCircle, Moon, Shield, Sparkles } from 'lucide-react';
...
const features = [
  ['安心して話せる居場所', '否定・説教・無理な励ましをしない、穏やかなコミュニティです。', Shield],
  ['自分のペースで参加', '読むだけ、時々話すだけでも大丈夫。参加の形は自由です。', Leaf],
  ['定期オンライン交流会', '少人数で、静かに話せる時間を用意します。', MessageCircle],
];

const faqs = [
  ['顔出しは必要ですか？', '必要ありません。ニックネームで参加できます。'],
  ['途中で退会できますか？', 'いつでも退会できます。無理に続ける必要はありません。'],
  ['医療サービスですか？', 'いいえ。こころの居場所は医療・診断・治療を行うサービスではありません。緊急時は医療機関や公的相談窓口をご利用ください。'],
    ...
    <main>
      <nav className="fixed top-0 z-50 w-full border-b border-white/60 bg-cream/80 backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6 md:px-10">
          ...
          <div className="hidden items-center gap-8 text-sm text-muted md:flex">
            <a href="#about">できること</a><a href="#price">料金</a><a href="#faq">FAQ</a><a href="#contact">お問い合わせ</a>
          </div>
      ...
      <section id="top" className="min-h-screen bg-cream px-6 pt-32 md:px-10">
        <div className="mx-auto grid max-w-7xl items-center gap-12 py-16 md:grid-cols-[0.9fr_1.1fr] md:py-24">
          <div>
            ...
            <p className="mt-8 max-w-xl text-lg leading-9 text-muted">同じ悩みを抱える仲間と、安心してつながれるオンラインコミュニティ。焦らなくていい。無理をしなくていい。あなたのペースで過ごせる居場所です。</p>
            ...
            <p className="mt-8 text-sm leading-7 text-muted">※医療・診断・治療を行うサービスではありません。緊急時は医療機関や相談窓口へご相談ください。</p>
          </div>
          <div className="relative">
            ...
            <div className="relative overflow-hidden rounded-large bg-white shadow-soft">
              <img src="https://images.unsplash.com/photo-1497366811353-6870744d04b2?q=80&w=1600&auto=format&fit=crop" alt="穏やかな室内" className="h-[430px] w-full object-cover md:h-[540px]" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="price" className="bg-cream px-6 py-24 md:px-10">
        <div className="mx-auto max-w-5xl text-center">
          ...
          <div className="mx-auto mt-12 max-w-md rounded-[36px] bg-white p-8 shadow-soft">
            ...
            <div className="mt-8"><span className="text-5xl font-semibold text-deep">¥1,980</span><span className="text-muted"> / 月</span></div>
            <ul className="mt-8 space-y-4 text-left leading-7 text-muted">
              <li>・会員限定コミュニティ参加</li><li>・月数回のオンライン交流会</li><li>・読むだけ参加OK</li><li>・いつでも退会可能</li>
            </ul>
      ...
      <section id="faq" className="bg-blue/60 px-6 py-24 md:px-10">
        <div className="mx-auto max-w-4xl">
          ...
          <div className="mt-12 space-y-4">
            {faqs.map(([q, a]) => <div key={q} className="rounded-3xl bg-white p-7 shadow-card"><h3 className="font-medium text-deep">Q. {q}</h3><p className="mt-3 leading-8 text-muted">A. {a}</p></div>)}
          </div>
        </div>
      </section>
      ...
      <section id="contact" className="bg-deep px-6 py-24 text-white md:px-10">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <p className="mx-auto mt-6 max-w-2xl leading-8 text-white/75">参加前の質問や不安なことがあれば、まずはお問い合わせください。</p>
      ...
      <footer className="bg-cream px-6 py-10 text-center text-sm text-muted md:px-10">
        <p>© 2026 こころの居場所</p>
        <div className="mt-4 flex justify-center gap-5"><a>利用規約</a><a>プライバシーポリシー</a><a>特定商取引法表記</a></div>
      </footer>
    </main>
  );
}
